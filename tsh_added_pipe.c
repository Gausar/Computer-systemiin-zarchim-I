...
void sigint_handler(int sig) 
{
    int pid = fgpid(jobs); /*pid - д PID -г олгоно */
    int jid = pid2jid(pid); /*jid is a map for pid*/

    /*Энд ажил эсвэл бүлэг ажил байгаа эсэхийг шалгана*/
    if(pid != 0){
        kill(-pid, SIGINT); /*foreground дээрх job-ийг зогсооно*/
        /*Хэрвээ sig нь 0-ээс их байвал job устана*/
        if(sig > 0){
            printf("Job [%d](%d) terminated by signal %d\n", jid, pid, sig);
            deletejob(jobs, pid);
        }
...
void sigtstp_handler(int sig) 
{
    int pid = fgpid(jobs); //pid - g PID bolgon tohiruulj bna
    int jid = pid2jid(pid); //jid ni pid - iin map
    
    /*Хэрвээ үргэлжилж байвал job-ийг шалгана.*/
    if(pid != 0){
        printf("Job [%d] (%d) Stopped by signal %d\n", jid, pid, sig);
        getjobpid(jobs, pid)->state=ST; /*pid-ийн ажлыг олоод тэрний төлөвийг зогсонги болгоно.*/
        kill(-pid, SIGTSTP); /*foreground job-ийг зогсооно*/
        /*- pid нь parent-ийн id болох process id-г өгнө*/
    }
  return;
}
